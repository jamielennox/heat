...
class CinderClientPlugin(client_plugin.ClientPlugin):
    ...
    def get_volume_api_version(self, interface):
    ...
    def _create(self):
        session = self._get_session('cinder')
        interface = self._get_client_option('cinder', 'endpoint_type')
        version, service_type = self.get_volume_api_version(interface)
